...
public class MagicOfBooks {
    ...
    public static void displayNewBook(User user) {
        if (isEmpty(user.getNewBook())) {
            System.out.println("List of new books is empty!");
        } else {
            for (int i = 0; i < user.getNewBook().length; i++) {
                Book newBook = user.getNewBook()[i];
                int index = i + 1;
                if (newBook == null) {
                    break;
                } else {
                    System.out.println(index + ". " + newBook.getName());
                }
            }
        }
    }
    ...
    public static void displayFavourite(User user) {
        if (isEmpty(user.getFavourite())) {
            System.out.println("List of favourite books is empty!");
        } else {
            for (int i = 0; i < user.getFavourite().length; i++) {
                Book favourite = user.getFavourite()[i];
                int index = i + 1;
                if (favourite == null) {
                    break;
                } else {
                    System.out.println(index + ". " + favourite.getName());
                }
            }
        }
    }
    ...
    public static void displayCompleted(User user) {
        if (isEmpty(user.getCompleted())) {
            System.out.println("List of new books is empty!");
        } else {
            for (int i = 0; i < user.getCompleted().length; i++) {
                Book completed = user.getCompleted()[i];
                int index = i + 1;
                if (completed == null) {
                    break;
                } else {
                    System.out.println(index + ". " + completed.getName());
                }
            }
        }
    }
     ...
     */
    public static boolean isEmpty(Book[] list) {
        int check = 0;

        while (check < list.length) {
            if (list[check] == null) {
                break;
            }
            check++;
